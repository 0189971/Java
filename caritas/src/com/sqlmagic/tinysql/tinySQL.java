...
public abstract class tinySQL {
    ...
    /**
     * Bandera de paro de grupo
     */
    private boolean groupBreak=false;
    /**
     * Bandera para seguir grabando
     */
    private boolean keepRecord=true;
    /**
     * Bandera de exdepuracion
     */
    private boolean exDebug=false;
    /**
     * Bandera de haciendo depuracion
     */
    private boolean performDebug=false;
    /**
     * tabla hash de funciones de grupo
     */
    private Hashtable groupFunctions;
    /**
     * Nueva linea
     */
    private String newLine = System.getProperty("line.separator");
    /**
     * Objeto de insercion de tabla
     */
    public static tinySQLTable insertTable=(tinySQLTable)null;
    /**
     * Objeto where de tabla
     */
    private tinySQLWhere wc;
    /**
     ...
     * 
     * @return devuelve un objeto de conjunto de resultados tsResulSet
     * 
    ...
    public tsResultSet sqlexec() throws tinySQLException {
        ...
        System.err.println( "Have no Statement, no connection and " +
     ...
     * 
     * @return devuelve el conjunto de resultados
     * 
     * @exception tinySQLException la excepcion que se propaga
     */
     ...
     * executed.
     * 
     * @param unObjeto comando SQL a ejecutar
     *
     * @return the ResultSet or null, if no result set was created
     * 
     * @exception tinySQLException la excepcion que se propaga
     */
    protected tsResultSet sql( Object unObjeto ) throws tinySQLException {
        /*
         *    Build the ResultSet
         */
        tsResultSet rs = null;
        ...
        Vector actions, columns, columnDefs;
        Vector values, columnContexts, columnAliases;
        String actionType, orderType, tableName;
        String statementType, byteString;
        ...
        groupBreak=false;
        keepRecord=true;
        statementType = unObjeto.getClass().getName();
        try {
            /*
             * Instantiate a new parser object which reads from the SQLStream. 
             * This should probably be changed to a String at some point. Note 
             * that parsing is only done once for a PreparedStatement.
             */
            actions = (Vector)null;
            if( statementType.endsWith("tinySQLPreparedStatement") ) {
                pstmt = (tinySQLPreparedStatement)unObjeto;
                pstmt.updateActions(actions);
                actions = pstmt.getActions();
                byteString = pstmt.getSQLString();
                bStream = (byte[])null;
                //if( pstmt.getSQLString() != (String)null ) {
                if( pstmt.getSQLString() != null ) {
                    bStream = pstmt.getSQLString().getBytes();
                    }
                } 
            else 
            if( statementType.endsWith("tinySQLStatement") ) {
                bStream = 
                    ((tinySQLStatement)unObjeto).getSQLString().getBytes();
                } 
            else {
            ...
            /*
             * The actions Vector consists of a list of Hashtables. Each of 
             * these action Hashtables contains elements required for a 
             * particular SQL statement. The following elements are used for 
             * various actions;
             *
             ...
             *
             * Hashtable     selectTables   Hashtable of tables in a SELECT 
             *                              action. 
             *
             * Vector        columns        A list of column names used by the
             *                              the action.
             *
             * Vector        columnContexts A list of Strings indicating the 
             *                              context for the elements in the 
             *                              columns Vector.
             *                              Values can be SELECT,ORDER,GROUP.
             *      
             * Vector        columnDefs     A list of column objects used by the 
             *                              CREATE TABLE and ALTER TABLE ADD 
             *                              actions.
             *            
             * Vector        values         A list of String values used in 
             ...
             *            
             * tinySQLWhere  whereClause    An object containing the where 
             *                              clause which can be updated and 
             *                              queried.
             */
            ...
            for( i=0; i<actions.size(); i++ ) {
                ...
                System.out.println( i + " : " + actionType );
                ...
                if( tableName != (String)null & 
                    !actionType.equals("DROP_TABLE") &
                    !actionType.equals("CREATE_TABLE") & 
                    !actionType.equals("INSERT") & 
                ...
                System.out.println( " Accion : " + actionString );
                if( debug ) {
                ...
                if( actionType.equals( "SELECT" ) ) {
                    ...
                    if ( (String)h.get( "DISTINCT" )!=null ) {
                        distinct = true;
                        }
                    rs = selectStatement( selectTables,columns,
                                      wc,orderType,distinct, unObjeto );
                    } 
                else 
                if( actionType.equals( "INSERT" ) ) {
                    ...
                    insertStatement( statementType, tableName, columns, 
                            values );
                    } 
                else 
                ...
                if( actionType.equals( "ALTER_ADD" ) ) {
                    /*        
                     * SQL ALTER TABLE ADD
                     */              
                    columnDefs = (Vector) h.get( "COLUMN_DEF" );
                ...
                if( actionType.equals( "ALTER_DROP" ) ) {
                    /*         
     ...
     * 
     * @param unasTablas la tabla hash
     * @param vColumnas el vector de columnas
     * @param tySQLW la clausula where
     * @param ot un string
     * @param distinct distinto de
     * @param stmt comando sql
     * 
     * @return devuelve el conjunto de resultados
     * 
     * @exception tinySQLException la excepcion que se propaga
     */
     protected tsResultSet selectStatement( Hashtable unasTablas, 
        Vector vColumnas, tinySQLWhere tySQLW, String ot, boolean distinct, 
            Object stmt ) throws tinySQLException {
        Hashtable tables;
        Vector tableList;
        tsResultSet jrs;
         ...
         */
        jrs = new tsResultSet(tySQLW, this);
        ...
        try {
            ...
            //String deDonde = "tinySQL.selectStatement() :\n";
            //System.out.println( deDonde + " " + sqle.getMessage() );
            Utils.log( "Caught SQLException while setting Fetchsize and " + 
                    "ResultSetType" );
         ...
         */
        for( i=0; i<vColumnas.size(); i++ ) {
            columnObject = (tsColumn)vColumnas.elementAt(i);
            /*
     ...
     *
     * @param jrs el conjunto de resultados
     * 
     * @throws tinySQLException la excepcion que se propaga
     */
     protected void contSelectStatement( tsResultSet jrs )
        ...
        /*
         * Create a hashtable to enumerate the tables to be scanned and 
         * initialize with the first table name.
         */
        ...
        while( level>0 ) {
           boolean levelFound = false;
            ...
            while( keys.hasMoreElements() ) {
                 ...
                 *  one currently being processed, grab it's primary key 
                 *  (the hashkey), flag levelfound, and break!
                 */
                if( currLevel==level ) {
            ...
            if( levelFound ) {
                 ...
                 * for tinySQL.  The concept involves creating a new 
                 * tinySQLCondition class, instances of which will be created 
                 * by tinySQLWhere. At least initially only conditions that are 
                 * equal to a constant will be considered. One of these 
                 * conditions will be stored inside the dbfFileTable object. 
                 * Calls to nextRecord would then have to add logic to check to 
                 * see if an index exists.  The presence of a complete index 
                 * would be indicated by a counter that was equal to the number 
                 * of rows in the table.  In that case a single get would 
                 * deliver the record numbers required for the where condition.
                 ...
                 */
                if( performDebug ) {
                    System.out.println("Selecting records from "
                        + jtbl.table);
                    }
                /*
                ...
                while( jtbl.nextRecord() ) {
                    ...
                    if( !jtbl.isDeleted() ) {
                        /*
                         * Evaluate the where clause for each column in the 
                         * table. If it is false, skip to the next row.  
                         * Otherwise, add the column value to the output record.
                         */
                        ...
                        while( cols.hasMoreElements() ) {
                            ...
                            if ( wc != (tinySQLWhere)null ) {
                                whereStatus = wc.evaluate( columnName, 
                                        columnString );
                                if ( whereStatus.equals("FALSE") ) {
                                    /*
                                     * This column value caused the where clause 
                                     * to be FALSE. Go to the next row in the 
                                     * table.
                                     */
                                    found = false;
                                    break;
                        ...
                        /*
                         * If no where condition has evaluated to false then 
                         * this record is a candidate for output. Break to the 
                         * next table in this case for further WHERE processing.
                         */
                        if( found ) {
                            break;
                            }
                ...
                if( found ) {
                    ...
                    eof = false;
                    /*
                     * If the table we are processing is not the last in the
                     * list, then we should increment level and loop to the top.
                     */          
                    if( level<t.size() ) {
                        /*
                         * Increment level
            ...
            if( haveRecord ) {
                /*
                 * If group functions are involved, add records only after a 
                 * break, which is defined as a change in all of the group 
                 * columns. Otherwise, update the current record.
                 */
                if( jrs.isGrouped() ) {
                    if( groupBreak ) {
        ...
        while( jtbl.nextRecord() ) {
            if (!jtbl.isDeleted()) {
                ...
                while( cols.hasMoreElements() ) {
                    ...
                    columnString = jtbl.getCol(columnName);
                    /*
                     * Check the status of the where clause for each column 
                     * value.
                     */
                    if( wc != (tinySQLWhere)null ) {
     ...
     * @param c el vector c
     * @param v el vector v
     * @param wc la clausula where
     * 
     * @throws tinySQLException la excepcion que se propaga
     */
     private void updateStatement( String tableName, Vector c, Vector v, 
         tinySQLWhere wc ) throws tinySQLException {
        /*
         * Create a table object and put it in the Hashtable.
         */
     ...
     * 
     * @param statementType el tipo de comando
     * @param tableName el nombre de la tabla
     * @param columns el vector columnas
     * @param values el vector valores
     * 
     * @throws tinySQLException la excepcion que se propaga
     */
     private void insertStatement( String statementType, String tableName,
        Vector columns, Vector values ) throws tinySQLException {
        ...
        System.out.println( "TABLA: " + insertTable + " " + statementType );
        /*
         * Check that the values supplied are the correct type and length.
         */
        for( i = 0; i < columns.size(); i++ ) {
            columnName = (String)columns.elementAt(i);
            System.out.println( "COLUMNA : " + columnName );
            valueString = (String)values.elementAt(i);
            System.out.println( "VALOR STRING A: " + valueString );
            if( valueString==(String)null ) {
                continue;
                }
            valueString = UtilString.removeQuotes(valueString);
            valueString = UtilString.replaceAll(valueString,"''","'");
            System.out.println( "VALOR STRING B: " + valueString );
            columnType = insertTable.colType(columnName);
            System.out.println( "COLUMNA TIPO : " + columnType );
            if( Utils.isNumberColumn(columnType) ) {
                try {
                    value = Double.parseDouble(valueString);
                    System.out.println( "VALOR : " + value );
                } catch ( Exception e ) {
                    String deDonde = "tinySQL.insertStatement() :\n";
                    throw new tinySQLException( deDonde + 
                        " Insert failed: column " + columnName + 
                            " is numeric - found " + valueString );
                }
                } 
            else 
            if( Utils.isDateColumn(columnType) ) {
                try {
                    /*
                     * Modify the input to be the standard YYYYMMDD format
                     */
                    if( valueString.trim().length() > 0 ) {
                        values.setElementAt(UtilString.dateValue(valueString),i);
                        }
                } catch( Exception e ) {
                    String deDonde = "tinySQL.insertStatement() :\n";
                    throw new tinySQLException( deDonde + 
            ...
            System.out.println( "TAM COLUMNA : " + columnSize );
        ...
        System.out.println( "SALI : insertando en tabla" );
        insertTable.insertRow( columns, values );
        /*
     ...
     * 
     * @param tableName el nombre de la tabla
     * @param v el vector que contiene los nombres a alterar
     * 
     * @exception tinySQLException la excepcion que se propaga 
     * @exception IOException la excepcion que se propaga 
     */
    public abstract void createTable( String tableName, Vector v ) 
     ...
     * @exception IOException la excepcion que se propaga 
     */
    public abstract void alterTableAddCol( String tableName, Vector v ) 
        throws IOException, tinySQLException;
    /**
     ...
     * @exception tinySQLException la excepcion que se propaga 
     */
    public abstract void alterTableDropCol( String tableName, Vector v ) 
        throws IOException, tinySQLException;
    /**
     * Rename columns
     *
     ...
     * 
     * @param tableName nombre de la tabal
     * @param oldColumnName nombre anterior de la columna
     * @param newColumnName nombre nuevo
     * 
     * @exception tinySQLException la excepcion que se propaga
     */
    public abstract void alterTableRenameCol( String tableName, 
        String oldColumnName, String newColumnName ) throws tinySQLException;
    /**
     * Drops a table by name
     * 
     * @param tableName nombre de la tabla que se borra
     * 
     * @exception tinySQLException la excepcion que se propaga
     */
    public abstract void dropTable( String tableName ) throws tinySQLException;
    /**
     ...
     * @exception tinySQLException la excepcion que se propaga
     */
    public abstract tinySQLTable getTable( String tableName ) 
            throws tinySQLException;
}
