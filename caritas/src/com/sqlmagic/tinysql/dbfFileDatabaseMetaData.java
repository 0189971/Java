 ...
 * @author Marcel Ruff <ruff@swand.lake.de> Added DatabaseMetaData with JDK 2 
 * support
 * @author Thomas Morgner <mgs@sherito.org> Changed DatabaseMetaData to use 
 * java.sql.Types.
...
public class dbfFileDatabaseMetaData extends tinySQLDatabaseMetaData {
    ...
    /**
     * Crea el objeto del archivo dbf y sus metadatos
     * 
     * @param connection la conexion a la BD
     */
    public dbfFileDatabaseMetaData( Connection connection ) {
        super(connection);
    }
    /**
     * Devuelve el nombre del gestor de la base de datos 
     * 
     * @return devuelve "tinySQL"
     */
    public String getDatabaseProductName() {
        return "tinySQL";
    }
    /**
     * Devuelve la cersion del gestor
     * 
     * @return devuelve 2.26
     */
    @Override
    ...
    /**
     * Devuelve el directorio de localizacion de los datos
     * 
     * @return devuelve el directorio de datos
     */
    ...
    public ResultSet getTypeInfo() throws SQLException {
        ...
        //       so i use the toString() method here Perhaps in future the 
        //       real type should be pushed into the Hashtable?

        tsRow record = new tsRow();
        // "CHAR", String
        record.put( "TYPE_NAME", dbfFile.typeToLiteral( Types.CHAR ) );    
        record.put( "DATA_TYPE", new Integer( Types.CHAR ).toString() );
        //record.put("PRECISION", new Integer( 254 ).toString() );
        record.put( "PRECISION", new Integer( 508 ).toString() );
        ...
        record = new tsRow();
        // "FLOAT", double
        record.put( "TYPE_NAME", dbfFile.typeToLiteral( Types.FLOAT ) );    
        ...
        record = new tsRow();
        // "CHAR", boolean "YyNnTtFf"
        record.put( "TYPE_NAME", dbfFile.typeToLiteral( Types.BIT ) );     
        ...
        record = new tsRow();
        // "INT", unsigned long
        record.put( "TYPE_NAME", dbfFile.typeToLiteral( Types.INTEGER ) );     
        ...
        record = new tsRow();
        // "DATE", date
        record.put( "TYPE_NAME", dbfFile.typeToLiteral( Types.DATE ) );     
     ...
     *              only "TABLE" is supported, others like "VIEW", 
     *              "SYSTEM TABLE", "SEQUENCE" are ignored.
     * @return ResultSet - each row is a table description
     * @exception SQLException if a database access error occurs
     * see #getSearchStringEscape
     *
     * author Thomas Morgner <mgs@sherito.org> Fill all needed columns, or 
     * some query tools will crash :(
     */
    public ResultSet getTables( String catalog, String schemaPattern,
        ...
        try {
         ...
         for( int itype=0; itype<types.length; itype++ ) {
            ...
            String deDonde = "dbfFileDatabaseMetaData.getTables():\n";
            System.out.println( deDonde + "Unable to create MetaData" );
    ...
    public ResultSet getColumns( String catalog, String schemaPattern,
        ...
        try {
            String dataDir = getDataDir();
            Utils.log( "Entering getColumns(tableNamePattern='" + 
                    tableNamePattern + "')" );
            if( dataDir==null ) {
                return null;
                }
            ResultSet tableRs = getTables( catalog, schemaPattern, 
            ...
            while( tableRs.next() ) { 
                ...
                try {
                    ...
                    String deDonde = "dbfFileDatabaseMetaData.getColumns():\n";
                    System.out.println( deDonde + " " + e.getMessage() );
                    // ignore buggy and empty (zero byte size) DBF files
                    continue; 
                }
                ...
                for( i = 0; i < tbl.columnNameKeys.size(); i++ ) {
                    ...
                    record.put( "COLUMN_NAME", 
                            tinySQLGlobals.getLongName( tsc.name ) );
                    record.put( "DATA_TYPE", 
                            new Integer(tsc.type).toString() );
                    record.put( "TYPE_NAME", 
                            dbfFile.typeToLiteral( tsc.type ).toString() );
                    record.put( "COLUMN_SIZE", 
                            new Integer(tsc.size).toString() );
                    record.put( "DECIMAL_DIGITS", 
                            new Integer(tsc.decimalPlaces).toString() );
                    ...
                    /*
                     * Suppress any sorting of the ResultSet. Column Metadata 
                     * should be presented in the order the columns exist in 
                     * the dBase header.
                     */
