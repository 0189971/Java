...
public class SimpleXMLTag extends Hashtable {
    ...
    /**
     * Indice que apunta a los campos
     */
    ...
    /**
     * indice de la ultima etiqueta
     */
    private int tagEndAt;
    /**
     * Eqiqueta de nombre XML
     */
    ...
    /**
     * Crea un objeto de etiqueta de XML
     * 
     * @param inputString cadena de entrada
     * @param startAt iniciar en
            ...
            if ( nextChar=='<' ) {
                if( tagName==(String)null ) {
                    ...
                    if ( tagNameEndAt < 0 ) {
                        ...
                        throw new Exception( "El Tag esta incompleto" );
                        }
                    tagName = inputString.substring(i + 1,tagNameEndAt);
                    if ( debug ) {
                        //System.out.println( "Found tag " + tagName );
                        System.out.println( "Tag encontrada " + tagName );
                        }
                    ...
                    for( j=tagNameEndAt+1; j<inputString.length(); j++ ) {
                        ...
                        startBodyAt = j;
                        if( debug ) {
                            //System.out.println("Start body at " + j);
                            System.out.println("Inicia cuerpo en " + j);
                            }
                        break;
                        }
                    } 
                    ...
                    if( i+tagName.length()+1>inputString.length()-1 ) {
                        ...
                        throw new Exception( "XML termino prematuramente." );
                        }
                    /*
                    ...
                    if ( testString.equals("/" + tagName) ) {
                        endBodyAt = i - 1;
                        if( debug) {
                            //System.out.println( "End body at " + endBodyAt );
                            System.out.println( "Final del cuerpo en " + 
                                endBodyAt );
                            }
                        bodyString = 
                            inputString.substring( startBodyAt, endBodyAt+1 );
                        tagEndAt = i + tagName.length() + 2;
                        if( debug) {
                            //System.out.println("Tag body is " + bodyString);
                            System.out.println( "Cuerpo Tag es " + bodyString );
                            }
                        //put( "TAG_NAME", tagName );
                        put( "TAG_NOMBRE", tagName );
                        ...
                        if( embeddedTagAt>-1 ) {
                            ...
                            if( embeddedTagAt>startBodyAt ) {
                                tagLabel = inputString.substring( startBodyAt, 
                                    embeddedTagAt).trim();
                            if( tagLabel.length()>0 ) {
                                if ( debug ) {
                                    //System.out.println( "label " + tagLabel );
     ...
     * 
     * @param inputIndex indice de entrada
     */
     ...
     * 
     * @return devuelve true si hay mas etiquetas
     */
    ...
    /**
     * busca el siguiente Tag
     * 
     * @return devuelve un objetp SimpleXMLTag 
     */
     ...
     * 
     * @return devuelve el estado del objeto convertido a String
     */
    @Override
    public String toString() {
        ...
        if( debug ) {
            //System.out.println("Tag object type is " + objectType);
            System.out.println( "Objeto Tag de tipo " + objectType );
            }
